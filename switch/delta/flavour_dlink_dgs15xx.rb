  ...
  module SwitchDelta
    ...
    class DlinkDeltaCommandRenderer < DeltaCommandRenderer
      def buildConfig(oldSwitchConfig, newSwitchConfig, delta)
        result = Construqt::Flavour::Ciscian::Result.new(Host.new("test-switch-123", "dlink-dgs15xx"))

        delta["addedBonds"].each do |addedBond|
          result.add("interface port-channel #{addedBond}", Construqt::Flavour::Ciscian::SingleValueVerb)
        end

        delta["removedBonds"].each do |removedBond|
          result.add("no interface port-channel #{addedBond}", Construqt::Flavour::Ciscian::SingleValueVerb)
        end
        ...
        delta["bondChanges"].each do |channel,bondDelta|
          bondDelta["addedPorts"].each do |addedPort|
            result.add("interface ethernet #{addedPort}") do |section|
              section.add("channel-group #{channel} mode active", Construqt::Flavour::Ciscian::SingleValueVerb)
            end
          end
        end
        ...
        (delta["addedPorts"] + delta["portChanges"].keys).each do |addedPort|
          ...
          if (allowedVlans.length + nativeVlans.length > 0)
            result.add("interface ethernet #{addedPort}") do |section|
              allowedVlans.each {|allowedVlan| section.add("switchport trunk allowed vlan", Construqt::Flavour::Ciscian::RangeVerb).add(allowedVlan) }
              nativeVlans.each {|nativeVlan| section.add("switchport trunk native vlan", Construqt::Flavour::Ciscian::RangeVerb).add(nativeVlan) }
            end
          end
        end
        ...
        delta["removedPorts"].each do |removedPort|
          result.add("interface ethernet #{removedPort}") do |section|
            oldSwitchConfig.portConfigs[removedPort].vlans.each do |vlanid,conf|
              ...
              section.add("no switchport trunk #{mode} vlan", Construqt::Flavour::Ciscian::SingleValueVerb).add(vlanid.to_s)
            end
          end
        end
        ...
        delta["removedVlans"].each do |removedVlan|
          result.add("no vlan #{removedVlan.to_s}", Construqt::Flavour::Ciscian::SingleValueVerb)
        end
