...
#    Construct.logger.debug("planuml.draw:#{node.reference.name} #{node.ident} ")
  ...
  def self.add_node_factory(type, host, *args)
    factory = {
      ...
      "DeviceDelegate.build_config" => lambda do |type, host, *args|
        #Construct.logger.debug("DeviceDelegate.build_config:#{host.class.name} #{args.map{|i| i.name}}")
        args.first
      end,
      "HostDelegate.build_config" => lambda do |type, host, *args|
        #Construct.logger.debug("Planuml:HostDelegate.build_config:#{host.name}")
        #binding.pry
        args.first
      end,
    ...
    if method
      ...
      Construct.logger.debug "Planuml:add_node_factory type not found #{type}"
    end
  end
  def self.build_tree
    #binding.pry
